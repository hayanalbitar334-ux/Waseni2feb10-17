...
export default function CartPage() {
  ...
  const [items, setItems] = useState([
    { id: 1, name: 'بيتزا مارغريتا كلاسيك', price: 50, quantity: 1, image: 'https://images.unsplash.com/photo-1574071318508-1cdbad80ad50?w=400&q=80', meta: 'حجم وسط • عجينة رقيقة' },
    { id: 2, name: 'سلطة سيزر بالدجاج', price: 35, quantity: 2, image: 'https://images.unsplash.com/photo-1550304943-4f24f54ddde9?w=400&q=80', meta: 'صوص إضافي • بدون خبز' },
  ]);
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-32">
      ...
      <header className="bg-white px-4 py-6 flex items-center justify-between sticky top-0 z-40">
        ...
        <button className="w-10 h-10 bg-red-50 text-red-500 rounded-full flex items-center justify-center">
          <Trash2 size={20} />
        </button>
      </header>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-emerald-600 font-bold mb-3">{item.meta}</p>
              <div className="flex items-center justify-between">
                <span className="text-emerald-600 font-black">{item.price}.00 ر.س</span>
                <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-1">
                  ...
                  <button className="w-8 h-8 bg-emerald-600 text-white rounded-lg flex items-center justify-center shadow-sm">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mt-6">
          ...
          <div className="space-y-3 mb-6">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-900 font-bold">{subtotal}.00 ر.س</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-400">رسوم التوصيل</span>
              <span className="text-gray-900 font-bold">{delivery}.00 ر.س</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-red-500">الخصم (WELCOME20)</span>
              <span className="text-red-500 font-bold">-{discount}.00 ر.س</span>
            </div>
          </div>
          <div className="pt-4 border-t border-dashed border-gray-100 flex justify-between items-center">
            <span className="text-xl font-black text-gray-900">الإجمالي</span>
            <span className="text-2xl font-black text-emerald-600">{total}.00 ر.س</span>
          </div>
        </div>
      </div>
